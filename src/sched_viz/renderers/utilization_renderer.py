...
class UtilizationRenderer:
    ...
    def render(self, vm: UtilizationViewModel, force_full=False, color_map=None) -> go.Figure:
        ...
        colors = [
            cfg.event_palette[0] if a.utilization >= vm.mean_utilization else cfg.event_palette[3]
            for a in visible
        ]
        fig = go.Figure()
        fig.add_trace(
        ...
        for value, label, color in [
            (vm.mean_utilization * 100, "Mean", cfg.event_palette[2]),
            (vm.median_utilization * 100, "Median", cfg.event_palette[6]),
        ]:
            fig.add_vline(
                x=value,
        ...
        sub = (
            f"<br><sup style='color:{cfg.text_secondary}'>Showing top {self._max_actors} of {len(vm.actors)}</sup>"
            if is_large
            else ""
        )
        ...
        fig.update_layout(
            ...
            title=dict(
                text=f"Actor Utilization{sub}", font=dict(size=15, color=cfg.text_primary), x=0.01
            ),
            showlegend=False,
            margin=dict(l=140, r=60, t=70, b=60),
            xaxis=dict(
                title=dict(text="Utilization (%)", font=dict(color=cfg.text_secondary)),
                range=[0, 105],
