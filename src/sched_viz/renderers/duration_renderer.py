...
class DurationRenderer:
    ...
    def render(self, vm: DurationViewModel, color_map=None) -> go.Figure:
        ...
        fig.add_vline(
            x=vm.mean_duration,
            line=dict(color=cfg.text_secondary, width=1.5, dash="dash"),
            annotation=dict(
                text=f"Mean: {vm.mean_duration:.1f}",
                font=dict(color=cfg.text_secondary, size=11),
        ...
        fig.update_layout(
            ...
            title=dict(
                text="Assignment Duration Distribution",
            ...
            xaxis=dict(
                title=dict(text="Duration (units)", font=dict(color=cfg.text_secondary)),
                gridcolor=cfg.grid_color,
                tickfont=dict(color=cfg.text_secondary),
                fixedrange=False,
            ),
            yaxis=dict(
                title=dict(text="Count", font=dict(color=cfg.text_secondary)),
                gridcolor=cfg.grid_color,
                tickfont=dict(color=cfg.text_secondary),
                fixedrange=False,
            ),
