...
class GanttTransformer:
    ...
    def transform(self, solution: Solution) -> GanttViewModel:
        color_map = self._external_color_map if self._external_color_map is not None                     else ColorRegistry(self._config).build(solution.event_ids)
        actor_order = self._build_actor_order(solution)
        bars = self._build_bars(solution)
        return GanttViewModel(bars=bars, actor_order=actor_order, color_map=color_map,
                              timeline_start=solution.timeline_start, timeline_end=solution.timeline_end)
    ...
    def _build_bars(self, solution):
        if not self._merge_bars:
            return [GanttBar(actor_id=a.actor_id, event_id=a.event_id,
                             start=a.start, end=a.end, label=a.event_id)
                    for a in solution.assignments]
        bars = []
        sorted_a = sorted(solution.assignments, key=lambda a: (a.actor_id, a.event_id, a.start))
        for (actor_id, event_id), group in groupby(sorted_a, key=lambda a: (a.actor_id, a.event_id)):
    ...
    def _merge_group(self, actor_id, event_id, assignments):
        ...
        for a in assignments[1:]:
            if a.start <= ce: ce = max(ce, a.end)
            else:
                merged.append(GanttBar(actor_id=actor_id, event_id=event_id, start=cs, end=ce, label=event_id))
                cs, ce = a.start, a.end
        merged.append(GanttBar(actor_id=actor_id, event_id=event_id, start=cs, end=ce, label=event_id))
        return merged
