...
class UtilizationTransformer:
    def transform(self, solution: Solution) -> UtilizationViewModel:
        ...
        for a in solution.assignments:
            ...
            cnt[a.actor_id] += 1
        actors = sorted([
            ActorUtilization(actor_id=aid, assigned_duration=total, timeline_length=tl,
                             utilization=min(total/tl, 1.0), n_assignments=cnt[aid])
            for aid, total in dur.items()
        ], key=lambda x: x.utilization, reverse=True)
        utils = [a.utilization for a in actors]
        n = len(utils)
        mean = sum(utils) / n
        mid = n // 2
        median = utils[mid] if n % 2 == 1 else (utils[mid-1] + utils[mid]) / 2
        return UtilizationViewModel(actors=actors, timeline_length=tl,
                                    mean_utilization=mean, median_utilization=median)
