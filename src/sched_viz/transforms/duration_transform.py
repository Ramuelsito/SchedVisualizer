...
class DurationTransformer:
    def transform(self, solution: Solution) -> DurationViewModel:
        ...
        for a in solution.assignments: by_event[a.event_id].append(a.duration)
        all_d = [a.duration for a in solution.assignments]
        return DurationViewModel(durations_by_event=dict(by_event), all_durations=all_d,
                                 min_duration=min(all_d), max_duration=max(all_d),
