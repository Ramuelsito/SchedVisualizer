...
class HeatmapTransformer:
    ...
    def transform(self, solution: Solution) -> HeatmapViewModel:
        ...
        bucket_size = max(1, round(span / AUTO_TARGET_BUCKETS)) if self._bucket_size == "auto" else int(self._bucket_size)
        n_buckets = max(1, (span + bucket_size - 1) // bucket_size)
        actor_ids = sorted(solution.actor_ids) if self._sort_actors == "alpha" else self._by_load(solution)
        if self._metric == "occupancy":    z = self._occupancy(solution, actor_ids, t_start, n_buckets, bucket_size)
        elif self._metric == "assignments": z = self._assignments(solution, actor_ids, t_start, n_buckets, bucket_size)
        else:                               z = self._events(solution, actor_ids, t_start, n_buckets, bucket_size)
        max_value = max((max(row) for row in z if row), default=1.0) or 1.0
        return HeatmapViewModel(z=z, actor_labels=actor_ids,
                                bucket_labels=[str(t_start + i*bucket_size) for i in range(n_buckets)],
                                bucket_size=bucket_size, max_value=max_value, metric=self._metric)
    ...
    def _occupancy(self, sol, actors, t0, nb, bs):
        ...
        for a in sol.assignments:
            for t in range(a.start, a.end):
                idx = (t - t0) // bs
                if 0 <= idx < nb: z[a.actor_id][idx] += 1.0
    ...
    def _events(self, sol, actors, t0, nb, bs):
        z = {a: [set() for _ in range(nb)] for a in actors}
        for a in sol.assignments:
            for idx in range(max(0,(a.start-t0)//bs), min(nb,(a.end-1-t0)//bs+1)):
